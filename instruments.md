...
# The XBT digitizer
<p><br>During the 1970s International Decade of Ocean Exploration (IDOE) the ubiquitous XBT became a powerful tool for probing the structure of the main thermocline because its ease of underway use and high vertical resolution (0.6 m) revealed structures hydrographic methods couldn’t resolve. The new and very accurate CTDs had the resolution but required one to several hours to obtain a single profile to depth. The XBT data were displayed on a military-grade graphic recorder for ease of use. Military-grade because the XBT was widely used by the navy for accurate real-time temperature data from which to infer the sound velocity structure. But the XBT was very useful for ocean science as well. 
